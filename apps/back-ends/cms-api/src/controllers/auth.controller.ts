  ...
  async signupWithEmailLink(@Body() signupDto: SignupWithEmailLinkDto) {
    try {
      ...
      const actionCodeSettings: ActionCodeSettings = {
        url: `${process.env.FRONTEND_CMS_URL}/auth/verify-email`,
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        emailLink = `${process.env.FRONTEND_CMS_URL}/auth/verify-email?oobCode=${verificationCode}&mode=verifyEmail&email=${encodeURIComponent(params.email)}&uid=${userUid}`;
      } else {
  ...
  async forgotPassword(@Body() forgotPasswordDto: ForgotPasswordDto) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        resetLink = `${process.env.FRONTEND_CMS_URL}/auth/reset-password?oobCode=${resetCode}&mode=resetPassword&email=${encodeURIComponent(email)}`;
      } else {
